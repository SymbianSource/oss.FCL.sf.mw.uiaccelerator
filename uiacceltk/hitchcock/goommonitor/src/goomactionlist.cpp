...
void CGOomActionList::AppNotExiting(TInt aWgId)
    ...
    while (index < iCloseAppActions.Count())
        ...
        TInt32 AppId1 = iMonitor.GetWindowGroupList()->AppIdfromWgId(action->WgId(), ETrue);
        TInt32 AppId2 = iMonitor.GetWindowGroupList()->AppIdfromWgId(aWgId, ETrue);
        if ( (AppId1 == AppId2) && (action->IsRunning()) )
            {
            TRACES2("CGOomCloseApp::AppNotExiting: App %x with window group id %d has responded to the close event", AppId1, aWgId);
