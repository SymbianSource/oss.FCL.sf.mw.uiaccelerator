...
private:    
	...
	*/
	...
	/**
 	 *  HandleSuppressVisibilityChangesL
	 *  
	 *  Notifies windows, whose position movement will not cause laboursome 
	 *  visibility recalculation. This can be used to optimize usecases
	 *  requiring high performance window movent.
	 */    
    void HandleSuppressVisibilityChangesL( TAlfBridgerData data );
	...
	*/    
	 ...
	 *
	 * @return ETrue if redirect is able to decide a target for the effect
	 *         EFalse if redirect cannot decide the effect target,
	 *				  i.e. there are multiple non-chained windowgroups
     */
    TBool HandleGfxRedirection(CFullScreenEffectState& aEvent, CHuiLayout*& aLayout);
	...
	*/
	...
	static TInt ComputeActualPosition(CHuiLayout* aLayout, TInt aChildPos);
	...
	void HandleBatchFadeAllChildren( TAlfBridgerData& aData );
