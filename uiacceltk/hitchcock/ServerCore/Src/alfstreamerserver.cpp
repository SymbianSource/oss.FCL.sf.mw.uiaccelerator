...
void CAlfStreamerServerSession::ServiceL(const RMessage2& aMessage)
   ...
   switch(op)
        ...
        case KAlfFlushDrawCommands:
            {
            // translate flush command to composition command
            TPtr8 dummy(0,0);
            TRAP_IGNORE(server->QueueRequestAllSessionsL(dummy, KAlfCompositionFlushDrawCommands, ETrue));
            break;
            }
        case EAlfSuppressVisibilityChanges:
            ...
            aMessage.Complete(KErrNone);    
            server->Bridge()->AddData( EAlfDSSuppressVisibilityChanges, windowGroupId, windowHandle, (TAny*)state );
            break;
            }
        case EAlfPostDataToCompositionClient:
            ...
            if (clientBufL > 0)
                { 
                buf = HBufC8::NewLC(aMessage.GetDesLength(2));
