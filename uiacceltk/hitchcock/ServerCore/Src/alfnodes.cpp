...
// ---------------------------------------------------------------------------
//
void CAlfNode::SetFaded( TBool aFaded, TAlfFadeBatch* aFadeBatch, TInt& aIndex )
    {
    ...
    if (iFadeState != iPostedFadeState)
        {
        TAlfFadeBatch& item = aFadeBatch[aIndex]; 
        item.iId = iId;
        item.iState = (TInt8)iFadeState;
        item.iType = (TInt8)iType;
        aIndex++;
...
void CAlfNode::BatchFadeAllChildren(TBool aFaded)
    {
    TInt index = 0;
    TInt offset = 0;
    TAlfFadeBatch* fadeBatch = (TAlfFadeBatch*)(CreateWindowAttributes(offset, iModel->iNodeHashArray.Count() * sizeof(TAlfFadeBatch)));
    FadeAllChildren(aFaded, fadeBatch, index);
    iModel->Server().Bridge()->AddData( EAlfFadeBatch, 
                        index, 
                        0, 
                        (TAny*)offset );
...
// ---------------------------------------------------------------------------
//
void CAlfNode::FadeAllChildren( TBool aFaded, TAlfFadeBatch* aFadeBatch, TInt& aIndex )
    {
    SetFaded(aFaded, aFadeBatch, aIndex);
    
    CAlfNode* node = (CAlfNode*)iSpriteChild;
    while( node )
        {
        node->SetFaded( iFadeState, aFadeBatch, aIndex );        
        node = node->iSibling;
        }
    ...
    if ( iTextCursor )
        {
        iTextCursor->SetFaded( iFadeState, aFadeBatch, aIndex );
        }
    node = (CAlfNode*)iChild;
    while ( node )        
        {
        node->FadeAllChildren( iFadeState, aFadeBatch, aIndex );
        node = node->iSibling;
        }
    }
