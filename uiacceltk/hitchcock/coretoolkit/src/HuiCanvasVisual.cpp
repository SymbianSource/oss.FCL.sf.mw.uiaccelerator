...
void CHuiCanvasVisual::Draw(CHuiGc& aGc) const
    ...
    if (EffectActive())
        ...
        if (EffectIsAppliedToChildren())
            ...
            TBool rootChanged = Changed();
            if(!refreshCache && rootChanged)
                {
                // if only root layout has change flag, force update all the renderbuffers
                // used by this effect
                // For example if a child window has been destroyed but nothing else
                // has changed and this destroyed window was the only window having
                // command buffers (content) we would draw incorrect old effect buffer
                // contents
                Effect()->ReleaseCachedRenderTarget();
                refreshCache = ETrue;
                }
...
EXPORT_C void CHuiCanvasVisual::SetChanged()
    {
    // don't mark all buffers changed if the visual changes, otherwise
    // it causes full redraw of the all buffers (even if not required)
...
EXPORT_C TInt CHuiCanvasVisual::CanvasFlags() const
    {
...
EXPORT_C void CHuiCanvasVisual::SetShape(const TPoint& aOrigin, const TRegion& aRegion)
    {
    // Check if the region *really* has changed
    TBool isReallyChanged = EFalse;
    TInt oldCount = iCanvasVisualData->iShapeRegion.Count();
    if ((oldCount != aRegion.Count()) || (aOrigin != iCanvasVisualData->iShapeOrigin))
        {
        isReallyChanged = ETrue;
        }
    else
        {
        for(TInt i=0; i<oldCount; i++)
            {
            if (iCanvasVisualData->iShapeRegion[i] != aRegion[i])
                {
                isReallyChanged = ETrue;
                break;
                }
...
void CHuiCanvasVisual::DrawExternalImage(CHuiGc& aGc) const
    ...
    if (Effectable()->ExternalTexture())
        ...
        TReal32 tlX = (displayArea.Width()/2.0) - (Effectable()->ExternalTexture()->Size().iWidth/2.0);
        TReal32 tlY = (displayArea.Height()/2.0) - (Effectable()->ExternalTexture()->Size().iHeight/2.0);
