...
void CGfxTransAdapterTfx::ConstructL()
    ...
    // If we get return values, we'll need a bigger buffer. The minimum size of 
    // HBufC8 descriptor should be 8 bytes
    iReturnBuffer = HBufC8::NewL( 2 * sizeof( TInt ) );
    ...
    // If we get return values, we'll need a bigger buffer. The minimum size of 
    // HBufC8 descriptor should be 8 bytes
    iAsyncReturnBuffer = HBufC8::NewL( 2 * sizeof( TInt ) );
