...
// Embedded application effects are not supported, apart from exception mentioned here
void CAlfServerDrawer::HandleAppFxExceptions(TUid aUid, TInt& aAction, TInt& aType, TInt& aData)
    {
    // Contact Card Application (CCA) is opened embedded from Favourites 
    // widget, Phonebook and Dialer.
    switch(aUid.iUid )
    {
        case KCCAApplicationId:
         {
        // note, that EndFullScreen request is skipped in case of CCA (EndFullScreen method)
        // NGA will detect endfullscreen more accurately.
        switch(aAction)
            {
            case AknTransEffect::EEmbeddedApplicationExit:
                {
                aAction = AknTransEffect::EApplicationExit;
                iEmbeddedApp = ETrue;
                break;
                }
            case AknTransEffect::EEmbeddedApplicationStart:
                {
                aAction = AknTransEffect::EApplicationStart;
                iEmbeddedApp = ETrue;
                break;
                }
            default:;
...
void CAlfServerDrawer::BeginFullscreen(	TInt aAction, const TRect& aEffectRect,
    ...
    HandleAppFxExceptions(aUid1, aAction, aType, aData);
    TInt previousAction = iFullScreenController->AppInfoCache().Action(aUid1);
    if ( previousAction == aAction 
            && previousAction !=  AknTransEffect::EApplicationExit 
            && aAction != AknTransEffect::ENone // let sequential enable/disable effects for appuid through
            )
        {
...
void CAlfServerDrawer::EndFullscreen(TBool aTimeout)
    {
    if (iEmbeddedApp && (iToUid.iUid == KCCAApplicationId))
        {
        // Skip EndFullScreen for embedded CCA application. It is coming too early. It is better let NGA deal with EndFullScreen detection.
        return;
        }
